...
class WallPost(BaseModel):
    ...
    user_name: str
    content: str
    category: str = "geral"  # geral, sugestao, discussao, aviso
    approved: bool = False
    pinned: bool = False
...
async def get_wall_posts(category: Optional[str] = None, current_user: User = Depends(get_current_user)):
    ...
    for p in posts:
        if isinstance(p.get('created_at'), str):
            p['created_at'] = datetime.fromisoformat(p['created_at'])
        # Ensure defaults for older posts
        p.setdefault('likes', [])
...
async def create_wall_post(post_data: WallPostCreate, current_user: User = Depends(get_current_user)):
    ...
    # Admin/moderator posts are auto-approved
...
@api_router.patch("/wall/{post_id}/pin")
...
@api_router.patch("/wall/{post_id}/like")
...
@api_router.post("/wall/{post_id}/comments")
...
@api_router.delete("/wall/{post_id}/comments/{comment_id}")
...
@api_router.get("/gallery/albums/{album_id}")
...
@api_router.post("/gallery/albums")
...
@api_router.patch("/gallery/albums/{album_id}")
...
@api_router.delete("/gallery/albums/{album_id}")
...
@api_router.post("/gallery/photos/upload")
