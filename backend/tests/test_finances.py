...
class TestTransactionsCRUD:
    ...
    def test_update_transaction(self, admin_session):
        ...
        create_payload = {
            "type": "receita",
            "category": "donativos",
            "description": "TEST_Donativo para Update",
            "amount": 5000.0,
        ...
        update_payload = {"description": "TEST_Doacao Atualizada", "amount": 7500.0}
...
class TestFinancialSummary:
    ...
    def test_get_summary_current_year(self, admin_session):
        ...
        print(
            f"Summary: Receitas={data['total_receitas']}, Despesas={data['total_despesas']}, Resultado={data['resultado_liquido']}"
        )
...
class TestFinanceSettings:
    ...
    def test_update_settings_admin_only(self, admin_session):
        ...
        update_response = admin_session.patch(
            ...
            json={"quota_amount": new_amount, "quota_description": "Quota Mensal Atualizada"},
        )
        assert update_response.status_code == 200, f"Failed: {update_response.text}"
...
class TestQuotaGeneration:
    ...
    def test_generate_quotas(self, admin_session):
        ...
        response = admin_session.post(f"{BASE_URL}/api/finances/generate-quotas", params={"month": 12, "year": 2026})
        assert response.status_code == 200, f"Failed: {response.text}"
        data = response.json()
    ...
    def test_generate_quotas_idempotent(self, admin_session):
        ...
        # First run
        response1 = admin_session.post(f"{BASE_URL}/api/finances/generate-quotas", params={"month": 11, "year": 2026})
        assert response1.status_code == 200
        created1 = response1.json()["created"]

        # Second run - should skip all
        response2 = admin_session.post(f"{BASE_URL}/api/finances/generate-quotas", params={"month": 11, "year": 2026})
...
class TestCategoriesMetadata:
    ...
    def test_get_categories(self, admin_session):
        ...
        # Verify statutory categories (Art. 5)
        assert "quotas" in data["income"]
        assert "joias" in data["income"]
        assert "extraordinarias" in data["income"]
        assert "patrocinios" not in data["income"]  # legada — migrada
        assert "operacional" in data["expense"]
        assert "receita" in data["types"]
        assert "despesa" in data["types"]
        # labels cobrem income + expense
        assert data["labels"].get("joias") == "Jóias"
