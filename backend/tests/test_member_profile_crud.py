...
def socio_token():
    ...
    response = requests.post(f"{BASE_URL}/api/auth/login", json={"email": SOCIO_EMAIL, "password": SOCIO_PASSWORD})
    if response.status_code == 200:
        return response.json()["access_token"]
...
class TestAdminUserUpdate:
    ...
    def test_admin_update_user_role_privileges(self, admin_headers):
        """PATCH /api/users/{userId} (admin) - update user role, privileges works"""
        ...
        # Update user (cargo é gerido em /admin/cargos, não aqui)
        update_data = {"role": "socio", "privileges": ["manage_events", "moderate_content"]}
        ...
        print(f"✓ Admin update successful: role={data['role']}, privileges={data['privileges']}")

    def test_admin_update_cargo_not_editable(self, admin_headers):
        """PATCH /api/users/{userId} (admin) - cargo não é editável aqui
        (spec-identidade-cargos: atribuído só via /admin/cargos). Enviar apenas
        cargo é ignorado, logo não sobra campo para atualizar -> 400."""
        # Get a user to update
        response = requests.get(f"{BASE_URL}/api/users?search=Carlos", headers=admin_headers)
        users = response.json()
        user_id = users[0]["id"]
        original_cargo = users[0].get("cargo")

        # cargo é ignorado (removido de UserAdminUpdate); sem outro campo -> 400
        response = requests.patch(
            f"{BASE_URL}/api/users/{user_id}", json={"cargo": "Presidente"}, headers=admin_headers
        )
        assert response.status_code == 400
        assert "Nenhum campo" in response.json().get("detail", "")

        # E mesmo junto de um campo válido, o cargo não muda.
        response = requests.patch(
            f"{BASE_URL}/api/users/{user_id}",
            json={"cargo": "Presidente", "phone_number": "999999999"},
            headers=admin_headers,
        )
        assert response.status_code == 200
        assert response.json().get("cargo") == original_cargo
        print("✓ Cargo não é editável via PATCH /users/{id} (só via /admin/cargos)")
    ...
    def test_admin_update_invalid_privileges_returns_400(self, admin_headers):
        ...
        response = requests.patch(
            f"{BASE_URL}/api/users/{user_id}", json={"privileges": ["invalid_privilege"]}, headers=admin_headers
        )
        assert response.status_code == 400
    ...
    def test_admin_update_invalid_role_returns_400(self, admin_headers):
        ...
        # Try invalid role
        response = requests.patch(
            f"{BASE_URL}/api/users/{user_id}", json={"role": "invalid_role"}, headers=admin_headers
        )
        assert response.status_code == 400
