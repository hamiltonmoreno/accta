...
```mermaid
timeline
    title Linha do tempo sugerida para licenciamento CTA em Cabo Verde
    section Pré-requisitos
      Idade mínima de 21 anos : Elegibilidade
      Certificado médico Classe 3 : Aptidão médica
      Inglês operacional mínimo nível 4 : Proficiência linguística
    section Formação inicial
      Matrícula em ATO aprovada pela AAC : Curso básico CTA
      Formação teórica aprovada : Conhecimentos obrigatórios
      Exame teórico : Aprovação
    section Formação operacional
      Formação de qualificação : ADI / APP / APS / ACP / ACS
      Serviço com tráfego real sob OJTI : Mínimo de 3 meses
      Avaliação prática / competência : Emissão
    section Ingresso em unidade
      Curso para averbamento de órgão : Unidade específica
      Autorização operacional : Posição de trabalho
      Recurrente / refrescamento : Plano de competências
    section Manutenção
...
```mermaid
flowchart TD
    A[Ocorrência operacional em órgão ATS] --> B[Mitigação imediata e continuidade segura do serviço]
    B --> C[Registo interno da ocorrência]
    C --> D[Preservação de documentos, gravações e evidências]
    D --> E[Notificação à AAC conforme regulamentação aplicável]
    E --> F[Recolha, tratamento e análise dos dados]
    F --> G[Identificação de tendências e deficiências]
    G --> H[Ações corretivas / formação / revisão de procedimentos]
    H --> I[Feedback para segurança operacional]

    D --> J{Acidente ou incidente grave?}
    J -- Sim --> K[Acionamento do regime de investigação técnica]
    K --> L[Interação com entidade investigadora competente]
    J -- Não --> I
```
