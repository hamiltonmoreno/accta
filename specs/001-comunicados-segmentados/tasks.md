...
- [ ] T007 Implementar `resolve_audience(audience_filter, *, channel, tipo)` em `backend/comunicados_service.py`: OR-dentro-do-tipo / AND-entre-tipos via intersecção de sets; órgãos via **`helpers.members_of_orgao`** (keys `direcao`/`mesa_ag`/`conselho_fiscal`, FR-012 — NB: a key inválida cairia no fallback de admins, por isso T003 fecha o enum); período por `admission_date` (None ⇒ não casa, R6); nominal por `member_id` **e** email (OR interno) com recolha dos não-encontrados; exclusão de `technical` incondicional após intersecção (FR-003); filtros de email (opt-out informativo, sem email) — depende de T003, T006
...
- [ ] T011 [P] Escrever `backend/tests/test_comunicados_audience.py`: OR-dentro/AND-entre-tipos; exclusão `technical`; período com `admission_date=None`; nominal inexistente recolhido; nominal **por email** resolve (C2); **órgão `mesa_ag` resolve para os membros da Mesa da AG — NÃO para a lista de admins (regressão de I2)**; `member_id` snapshot estável (usar `mock_db`, wire `comunicados`/`users`) — cobre T003/T007
