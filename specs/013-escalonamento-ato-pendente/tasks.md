...
- [x] T001 Confirmar a branch `feature/escalonamento-ato-pendente` ativa; confirmar que NÃO se adicionam dependências (não tocar `backend/requirements.txt`) nem há mudanças de schema/modelos (reutiliza `Ato.overdue_notified_at` e `FinanceSettings.ato_overdue_dias` das specs 010/012).
