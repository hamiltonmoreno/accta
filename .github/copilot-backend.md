...
├── models.py                 # Pydantic models (User, Invoice, Poll, etc.); re-exports CARGOS/PRIVILEGES from governance
├── auth.py                   # JWT creation/validation, password hashing, password_changed_at invalidation
├── governance.py             # Órgãos sociais, cargos, member categories, privileges (source of truth)
├── permissions.py            # RBAC / eligibility helpers (is_mesa_ag, is_direcao, is_voting_member…)
├── helpers.py                # Shared utilities (email, QR, PDF generation, notifications, audit)
├── email_service.py          # Resend integration (invite, reset, welcome, contacto, comunicados)
├── requirements.txt          # Python dependencies
│
├── routes/                   # 32 FastAPI routers (organized by domain)
│   ├── __init__.py           # Router aggregation (includes all module routers)
│   ├── auth_routes.py        # /api/auth/* (login, logout, password reset, setup account)
│   ├── admin.py              # /api/admin/* (user mgmt, registration approval, cargo mgmt)
│   ├── users.py              # /api/users/* (profiles, member listings)
│   ├── governance.py         # /api/governance/* (structure, corpos sociais)
│   ├── assembleias.py        # /api/assembleias/* (convening, attendance, quorum, deliberations, voting)
│   ├── eleicoes.py           # /api/eleicoes/* (lists, candidacy, secret ballot, proclamation)
│   ├── sancoes.py            # /api/sancoes/* (disciplinary sanctions)
│   ├── atos.py               # /api/atos/* (co-approval of financial acts, Art. 54)
│   ├── finances.py           # /api/finances/* (transactions, quotas, jóia, DRE, cash flow)
│   ├── invoices.py           # /api/invoices/* (billing, PDF/CSV export)
│   ├── prestacao_contas.py   # /api/* (exercícios, balancetes — annual account cycle)
│   ├── regulamentos.py       # /api/regulamentos/* (internal regulations versioning)
│   ├── comunicados.py        # /api/comunicados/* (email + in-app announcements)
│   ├── participacao.py       # /api/participacao/* (petições, propostas AG, reclamações, esclarecimentos)
│   ├── profissional.py       # /api/profissional/* (Cat 5: formações, publicações, defesa, relações)
│   ├── public_profissional.py# /api/public/* (public Cat 5 data, no auth)
│   ├── ranking.py            # /api/ranking/* (member participation ranking)
│   ├── polls.py              # /api/polls/* (voting)
│   ├── posts.py              # /api/posts/* (news/blog)
│   ├── documents.py          # /api/documents/* (institutional docs + access control)
│   ├── benefits.py           # /api/benefits/* (club benefits, QR validation, partners)
...
│   ├── notifications.py      # /api/notifications/* (REST + SSE stream, broadcast, audit logs)
│   ├── projects.py           # /api/projects/* (CRUD, tasks, milestones, budget)
│   ├── banners.py            # /api/banners/* (page banners)
│   ├── brand.py              # /api/brand/* (logo/brand settings)
│   ├── contact.py            # /api/contact/* (contact form → email)
│   ├── activity.py           # /api/activity/* (activity feed)
│   ├── report.py             # /api/report/* (personal activity reports)
